...
class fine_database{
    public:
        map<string, int> fine_map;
};

fine_database fine_db;
    ...
    public:
        ...
        void add_user(){
            ...
            for(int i=0; i<list_of_users.size(); i++){
                if(list_of_users[i].id == new_user.id){
                    system("CLS");
        ...
        void update_user(){
            ...
            string new_name, new_password, new_id;
            int type;
            int flag=0;
            switch(choice){
            case 1:
        ...
        //     map<string,vector<string>> issued_books_list=book_db.get_issued_books;
        //     if(issued_books_list[CurrentUser].size()==0){
        //         cout<<"No books issued!"<<endl;
        //         return;
        //     }
        //     for(int i=0; i<issued_books_list[CurrentUser].size(); i++){
        //         cout<<"Book name: "<<issued_books_list[CurrentUser][i]<<endl;
        //     }
        // }
        void display_issued_book();
}; 
...
class book_database{
    public:
        ...
        map<string,vector<string>> get_issued_books;
        ...
        void add_test_books(){
            for(int i=1; i<=6; i++){
                ...
                b.ISBN = "ISBN"+to_string(i);
                b.publication = "Publication"+to_string(i);
                b.issued = 0;
            ...
            for(int i=0; i<list_of_books.size(); i++){
                if(list_of_books[i].name == issue_book_name){
                    ...
                    if(list_of_books[i].issued == true){
                        ...
                        //cout<<type<<" "<<issued_books_list[CurrentUser].size()<<endl;
                        if(type == 1 && get_issued_books[user_db.CurrentUser].size() >= 5){
                            cout << "Student can't issue more than 5 books!" << endl;
                            return;
                        }
                        list_of_books[i].issued = true;
                        list_of_books[i].issued_by = user_db.CurrentUser;
                        list_of_books[i].issued_date = issue_date;
                        get_issued_books[user_db.CurrentUser].push_back(list_of_books[i].name);
            ...
            for(int i=0; i<list_of_books.size(); i++){
                if(list_of_books[i].name == return_book_name){
                    if(list_of_books[i].issued == false){
                        cout << "Book not issued by you!" << endl;
                        return;
                    }
                    else{
                        int d, m, y;
                        ...
                        //cout<<diff<<" "<<type<<endl;
                        if(type==1 && diff>30){
                            fine_db.fine_map[user_db.CurrentUser] = (diff-30)*2;;
                        }

                        if(type==0 && return_date.difference(list_of_books[i].issued_date)>60){
                            fine_db.fine_map[user_db.CurrentUser] = (diff-60)*5;
                        }
                        
                        list_of_books[i].issued_date = Date(0,0,0);
                        int it=0;
                        for(int j=0; j<get_issued_books[user_db.CurrentUser].size(); j++){
                            if(get_issued_books[user_db.CurrentUser][j] == list_of_books[i].name){
                                it = j;
                                break;
                            }
                        }
                        get_issued_books[user_db.CurrentUser].erase(get_issued_books[user_db.CurrentUser].begin()+it);
...
void user_database::display_issued_book(){
            cout<<"Display Issued Book Menu"<<endl;
            map<string,vector<string>> issued_books_list=book_db.get_issued_books;
            if(issued_books_list[CurrentUser].size()==0){
                cout<<"No books issued!"<<endl;
...
class professor: public users{
    ...
    public:
        professor(users p){ 
            name = p.name;
            id = p.id;
            fine = fine_db.fine_map[user_db.CurrentUser];
            type=0;
        }
        void search_book(){
            book_db.search_book();
        ...
        void show_issued_book(){
            cout<<"Display Issued Book Menu"<<endl;
            for(int i=0; i<book_db.get_issued_books[id].size(); i++){
                cout<<"Book name: "<<book_db.get_issued_books[id][i]<<endl;
            }
        }
};
...
void Profmenu(professor &prof){ 
    ...
    switch(choice){
       ...
       case 5:
            ...
            break;
        case 6:
           cout<<"Fine = "<<prof.check_fine()<<endl;
           break;
        case 7:
            prof.clear_fine();
            break;
...
void menu(){
    ...
    switch(choice){
        case 1:
            ...
            for(int i=0; i<user_db.list_of_users.size(); i++){
                if(user_db.list_of_users[i].id==user_id && user_db.list_of_users[i].password==password && user_db.list_of_users[i].type==1){
                    cout<<"Welcome "<<user_db.list_of_users[i].name<<endl;
                    cout<<"You have sucefully logged in!"<<endl;
                    user_db.CurrentUser=user_id;
                    flag=1;
                    Student Std(user_db.list_of_users[i]);
                    Studentmenu(Std);
                }
            }
        ...
        case 2:
            ...
            getline(cin, password);
            for(int i=0; i<user_db.list_of_users.size(); i++){
                if(user_db.list_of_users[i].id==user_id && user_db.list_of_users[i].password==password && user_db.list_of_users[i].type==0){
                    cout<<"Welcome "<<user_db.list_of_users[i].name<<endl;
                    cout<<"You have sucefully logged in!"<<endl;
                    user_db.CurrentUser=user_id;
                    flag=1;
                    professor p(user_db.list_of_users[i]);
                    Profmenu(p);
                }
            }
            if(flag==0) cout<<"Invalid user id or password!"<<endl;
        ...
        case 3:
            ...
            getline(cin, password);
            for(int i=0; i<user_db.list_of_users.size(); i++){
                if(user_db.list_of_users[i].id == user_id && user_db.list_of_users[i].password == password && user_db.list_of_users[i].type == 2){
                    cout<<"Welcome "<<user_db.list_of_users[i].name<<endl;
                    cout<<"You have sucefully logged in!"<<endl;
                    flag=1;
                    librarian l(user_db.list_of_users[i]);
                    Librarianmenu(l);
                }
            }
            if(flag==0) cout<<"Invalid user id or password!"<<endl;
...
int main(){
    ...
    user_db.add_test_student();
    user_db.add_test_professor();
    book_db.add_test_books();
